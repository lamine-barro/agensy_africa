...
const normalizeSchedule = (schedule = {}) => {
  if (!['asap', 'scheduled', 'recurring'].includes(schedule.type)) return null;
  ...
  if (schedule.type === 'recurring' && !['weekly', 'monthly'].includes(schedule.frequency)) return null;
  return schedule;
};
...
app.post('/v1/orders', auth, role('customer'), async (req, res, next) => { try {
  const { productId, quantity, deliveryAddress, schedule = { type: 'asap' }, instructions = '' } = req.body || {}; const product = await db.product(productId); const normalizedSchedule = normalizeSchedule(schedule);
