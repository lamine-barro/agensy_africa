...
app.post('/v1/orders', auth, role('customer'), async (req, res, next) => { try {
  const { productId, quantity, deliveryAddress, schedule = { type: 'asap' }, instructions = '' } = req.body || {}; const product = await db.product(productId);
  if (!product || !Number.isInteger(quantity) || !deliveryAddress?.label) return res.status(422).json({ error: 'INVALID_ORDER' }); if (quantity < product.minQuantity || quantity > product.maxQuantity) return res.status(422).json({ error: 'QUANTITY_OUT_OF_RANGE', min: product.minQuantity, max: product.maxQuantity });
  const now = new Date().toISOString(); const order = { id: db.id('ord'), customerId: req.user.sub, product, quantity, deliveryAddress, schedule, instructions: String(instructions).slice(0, 1000), status: 'draft', pricing: calculate(product, quantity), createdAt: now, updatedAt: now, timeline: [] }; res.status(201).json(await db.createOrder(order));
} catch (error) { next(error); } });
